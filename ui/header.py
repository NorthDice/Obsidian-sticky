...
class HeaderBar(Gtk.Box):
    def __init__(self, on_add, on_pin, on_close):
        ...
        icon = self._make_svg_image(OBSIDIAN_SVG)
        if icon:
        ...
        btn_add = self._make_btn("＋", "Add notes from Obsidian vault", on_add)
        btn_add.set_name("btn-add")
        self.pack_start(btn_add, False, False, 0)
        ...
        btn_close = self._make_btn("✕", "Close widget", on_close)
        btn_close.set_name("btn-close")
        self.pack_start(btn_close, False, False, 2)
    ...
    def set_pinned(self, pinned):
        if pinned:
            ...
            self._btn_pin.set_tooltip_text("Unpin — allow floating above windows")
        else:
            self._btn_pin.set_label("○")
